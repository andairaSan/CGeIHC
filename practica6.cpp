...
int main()
{
	...
	dadoTexture = Texture("Textures/cubo.tga");
	...
	Octaedro_M = Model();
	Octaedro_M.LoadModel("Models/octaedro.obj");
	...
	while (!mainWindow.getShouldClose())
	{
		...
		model = glm::scale(model, glm::vec3(0.1f, 0.1f, 0.1f)); // Ajusta la escala según necesites
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		octaedroTexture.UseTexture();
		Octaedro_M.RenderModel();
