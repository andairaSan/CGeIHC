...
int main()
{
	...
	while (!mainWindow.getShouldClose())
	{
		...
		// Movimiento del helicóptero hacia adelante (T) y atrás (G)
		if (mainWindow.getsKeys()[GLFW_KEY_T]) {
			heliX -= 0.5f * deltaTime; // Avanza
		}
		if (mainWindow.getsKeys()[GLFW_KEY_G]) {
			heliX += 0.5f * deltaTime; // Retrocede
		}
		...
		//Sigue al helicóptero y apunta hacia abajo
		glm::vec3 posHelicoptero = glm::vec3(heliX, 5.0f, 6.0f);
