...
float reproduciranimacion, habilitaranimacion, guardoFrame, reinicioFrame, contador = 0;
float ciclo = 0, ciclo2 = 0, ciclo3 = 0, ciclo4 = 0, ciclo5 = 0;
// luz direccional
...
float rotAla = 0.0f;       
float velAla = 150.0f;     // Velocidad 
bool alaSube = true;       // Bandera para saber si el ala va hacia arriba o abajo
...
void resetElements(void) //Tecla 0, reprouce la animación desde el inicio, regresa a la posición del primer frame
{
...
void interpolation(void)//el valor del frame siguiente menos el valor del frame actual entre el número de pasos
// para obtener el incremento que se le sumará al frame actual en cada iteración
{
...
void animate(void) 
{
	//Movimiento del objeto // barra espaciadora, tecla espacio activa la bandera de reproducción de la animación
	if (play)
	{
...
int main()
{
	...
	Ala1_M = Model();
	Ala1_M.LoadModel("Models/ala1.obj");
	...
	//KEYFRAMES DECLARADOS INICIALES, ya no son volátiles
	...
	while (!mainWindow.getShouldClose())
	{
		...
		if (play) // Solo aletea cuando la animación se está reproduciendo
		{
			if (alaSube) 
			{
				rotAla += velAla * deltaTime;
				if (rotAla > 20.0f) alaSube = false; // Límite superior del aleteo
			} 
			else 
			{
				rotAla -= velAla * deltaTime;
				if (rotAla < -20.0f) alaSube = true; // Límite inferior del aleteo
			}
		}
...
void inputKeyframes(bool* keys)
{
	...
	//Tecla 6: HABILITAR TODOS LOS MOVIMIENTOS
	if (keys[GLFW_KEY_6])
	{
		ciclo = 0;
		ciclo2 = 0;
		...
		ciclo4 = 0;
		ciclo5 = 0;
		printf("Movimientos habilitados (Ya puedes usar 1, 2, 3, 4 y 5 de nuevo)\n");
	}
